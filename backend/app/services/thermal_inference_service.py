...
from modules.thermal.preprocessing import decode_thermal_image


class UnsupportedThermalAssetTypeError(ValueError):
    pass
...
class ThermalInferenceService:
    ...
    def validate_asset_type(self, asset_type: str) -> None:
        if asset_type.strip().lower() not in self._supported_asset_types:
            ...
            raise UnsupportedThermalAssetTypeError(
                f"Thermal model supports these asset types: {supported}"
            )

    async def predict(self, machine_id: UUID, content: bytes) -> Prediction:
        image = decode_thermal_image(content)
        return await self._orchestrator.predict(machine_id, Modality.THERMAL, image)
